...
class ComparativeRankingSerializer(serializers.Serializer):
    ...
    stream_image = serializers.SerializerMethodField(read_only=True)
    stream_date = serializers.DateTimeField(read_only=True)
    ...
    @staticmethod
    def get_stream_image(instance):
        image = instance.get("stream_image")
        if not image:
            return None
        return f"{settings.MEDIA_URL}{image}"
