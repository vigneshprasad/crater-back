...
def create_base_sources():
    for source_name, score in constants.BASE_SOURCE_TO_SCORE_MAP.items():
...
def create_initial_sources():
    for link, source in constants.TYPEFORM_URL_TO_SOURCE_MAP.items():
...
def map_users_old_source_to_new_sources(users=None):
    ...
    for user in all_users:
        ...
        new_source = constants.EXISTING_SOURCES_TO_NEW_SOURCE_MAP.get(old_source, ("Organic", "Organic")) if old_source else ("Organic", "Organic")
        base_source, _ = models.BaseSource.objects.get_or_create(
            name=new_source[0]
        )
        source, _ = models.Source.objects.get_or_create(
