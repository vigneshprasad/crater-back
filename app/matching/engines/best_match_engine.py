"""This engine creates score for a user matching with another user.

This engine create a score for users matching with each other
without taking in to consideration the matched user's preferences.
...
def get_objective_to_objective_score_for_users(user, user2):
    ...
    return score_for_users * constants.DEFAULT_OBJECTIVE_MULTIPLIER
...
def get_intro_score_for_users(user, user2):
    ...
    if not (user.has_profile and user2.has_profile):
        return 0
    ...
    # Handling small intros 
    if user2_intro_len > constants.INTRO_REDUCTION_LENGTH:
        intro_len_factor2 = 1
    else:
        # The number 10 has been chosen arbitrarily here and can be
        # potentially tweaked. Current idea is that it is a sigmoid 
        # function with a mean chosen as half of 20. 
        intro_len_factor2 = 1 / (1 + numpy.exp(-(user2_intro_len / (constants.INTRO_REDUCTION_LENGTH / 2))))
